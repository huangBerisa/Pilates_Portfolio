...
- 設計規範: [LINEA Pilates UI/UX Design Specification v1.0](docs/DESIGN_SPEC_v1.0.md)（配色・タイポ・余白・
  CRAP四原則・Nielsen 10原則の一次資料。Figma と食い違う場合はこちらを優先）
...
- 見た目の一次資料は Figma。[docs/DESIGN_SPEC_v1.0.md](docs/DESIGN_SPEC_v1.0.md) は設計思想の参考資料
...
| 画面 | Figma ノード | 操作 |
| --- | --- | --- |
| ホーム（予約なし） | `HomepageSection` 175:797 | 日付タブ切替 / 店舗フィルタ / さらに表示 / 予約へ |
| ホーム（予約あり） | `予約済み` 182:778 | 予約枠スライダー（左右ドラッグ）/ 予約取り消し / 予約数カウント |
...
- 「ご予約中レッスン」は**上限3枠のスライダー**。左右にドラッグ（マウス／タッチ／← →キー）でき、
  見出し横の「N件 / 3件」カウンタとドットは同じデータから算出しているので予約状況と必ず一致する。
- 予約は **3件まで**。上限時は予約ボタンが無効になりトーストで通知。
...
```
src/
├── styles/tokens.css       デザイントークン（設計規範 §3〜§7 と1対1）
├── styles/components.css   コンポーネントのスタイル（Figmaノード名をコメント併記）
├── components/             Figma のコンポーネント単位で分割
├── components/ReservationSlots.tsx 予約枠スライダー（ドラッグ＋件数連動）
