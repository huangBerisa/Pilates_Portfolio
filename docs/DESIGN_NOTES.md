...
**レイアウト・配色・タイポの一次資料は Figma。**
[DESIGN_SPEC_v1.0.md](./DESIGN_SPEC_v1.0.md) は設計思想の参考資料として同梱しているが、
見た目の指定が Figma と食い違う場合は Figma を優先する。
（一度 SPEC の値で組み直したが、Figma の版のほうが読みやすいとの判断で差し戻した）
...
「ご予約中レッスン」は上限3枠のスライダー。`src/components/ReservationSlots.tsx`。
Figma の「予約カード」(182:801) の横並びと「Card / Add Booking Card」(210:1320) を組み合わせている。

- 枠は常に3つ並び、左右にドラッグして送れる
  （マウスはポインタドラッグ、タッチはネイティブスクロール、キーボードは ← → キー）
...
4. **プログラム数**
   Figma の3プログラム（Control Flow / Shape up Core / Refresh Core）に揃えている。
   写真素材が3点のため、デモ用に足していた Basic Align は削除した。
...
- **写真・ロゴ**: Figma から書き出した実素材を `public/images/` に配置済み。
  すべて Figma のスロット寸法どおりなのでトリミングは起きない。詳細は
  [FIGMA_ASSETS.md](./FIGMA_ASSETS.md)。
